...
function displayStudents(students) {
  ...
  students.forEach((student) => {
    ...
    // Create button to make student a prefect
...
function makeStudentPrefect(student) {
  // Find the existing prefect for the student's house (if any)
  const existingPrefect = students.find(
    (s) => s.house === student.house && s.prefect === true
  );

  // If there is an existing prefect, remove their prefect status
  if (existingPrefect) {
    existingPrefect.prefect = false;
  }

  // Set the student as the new prefect
